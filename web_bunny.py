...
def target():
    #gets input from forms (forms.py)/target.html
    form = url_form()
    ...
    if form.validate_on_submit():
        ...
        if (request_type=='POST'):
            ...
            if (error == True):
                return render_template('target.html',form=form,r=error_reply)
            elif(error == False):
                return render_template('target.html',r=r,form=form, headers = request_headers, reply = request_reply ,text = request_text, url=url, req=request_type)
        ...
        if (request_type=='OPTIONS'):
            ...
            if (error == True):
                ...
                return render_template('target.html',r=r,form=form, headers = request_headers, reply = request_reply ,text = request_text, url=url, req=request_type)
    return render_template('target.html',form=form,r=error)
